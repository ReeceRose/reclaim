...
import { type AppEvent, type ScanProgress, wsURL } from "@/lib/api";
          ...
          if (event === "scan_completed") {
            ...
            if (errors > 0) {
              const desc = [
                ...
                .filter(Boolean)
                .join(" · ");
              toast.warning("Scan completed with errors", {
                description: desc,
              });
            } else {
              toast.success("Scan complete", {
                description: parts.length ? parts.join(", ") : "No changes",
              });
            }
