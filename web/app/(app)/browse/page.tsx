...
function sortShows(
  ...
  const s = [...shows];
  if (sort === TV_SORT.SAVINGS)
    return s.sort(
      (a, b) => b.predicted_savings_bytes - a.predicted_savings_bytes,
    );
  if (sort === TV_SORT.SIZE)
    return s.sort((a, b) => b.total_bytes - a.total_bytes);
  if (sort === TV_SORT.FILES)
    return s.sort((a, b) => b.file_count - a.file_count);
  return s;
}
    ...
    <div className="bg-surface border border-line rounded-xl overflow-hidden">
      ...
      {Array.from({ length: 8 }).map((_, i) => (
        <div
          ...
          key={i}
          className="flex items-center gap-3 px-4 py-2.5 border-b border-line-soft last:border-b-0"
        >
          <Skeleton className="h-3 flex-1" />
          <Skeleton className="h-3 w-16" />
        ...
        <div className="bg-surface border border-line rounded-xl overflow-hidden">
          ...
          {shows.map((s) => (
            <ShowRow
              key={s.title}
              show={s}
              href={BROWSE_ROUTES.TV_SHOW(s.title, view)}
            />
          ))}
        </div>
      ) : (
...
function BrowsePage() {
  ...
  const [tvSortRaw, setTVSort] = useQueryParam(
    QUERY_PARAMS.TV_SORT,
    TV_SORT.ALPHA,
  );
  const tvSort = parseQueryEnum(
    tvSortRaw,
    TV_SORT_OPTIONS.map((o) => o.value),
    TV_SORT.ALPHA,
  );

  const [movieSortRaw, setMovieSort] = useQueryParam(
    QUERY_PARAMS.MOVIE_SORT,
    MOVIE_SORT.RECENT,
  );
  const movieSort = parseQueryEnum(
    movieSortRaw,
  ...
  const view = parseQueryEnum(
    viewRaw,
    [VIEW_MODE.GRID, VIEW_MODE.LIST] as const,
    VIEW_MODE.GRID,
  );
