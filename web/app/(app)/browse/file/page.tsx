...
function FilePageContent() {
  ...
  const {
    ...
    queryKey: ["file", id],
    // biome-ignore lint/style/noNonNullAssertion: query only runs when `enabled` confirms id is a valid number
    queryFn: () => api.file(id!),
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full py-32 text-center">
        ...
        <div className="text-xs text-muted-dim mt-2 mb-5">
          This file could not be found in the library.
        </div>
        <button
          type="button"
          onClick={() => goBack(router)}
  ...
  const metadataKey = isTV
    ? (file.path.split("/").at(-3) ?? displayTitle)
    : displayTitle;
    ...
    <div className="flex flex-col min-w-0 h-screen overflow-hidden max-sm:h-full">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                ...
                {file.vote_average != null && file.vote_average > 0 && (
                  <span className="text-xs text-muted-dim">
                    ★ {file.vote_average.toFixed(1)}
                  </span>
                )}
                {file.runtime_mins != null && (
                  <span className="text-xs text-muted-dim">
                    {file.runtime_mins} min
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center gap-1.5 flex-wrap text-xs text-muted-fg">
                ...
                {file.candidate_state === "candidate" &&
                  file.predicted_savings_bytes > 0 && (
                    <>
                      <span className="text-muted-dim">·</span>
                      <span className="text-brand font-semibold">
                        -{formatBytes(file.predicted_savings_bytes)} recoverable
                      </span>
                    </>
                  )}
              </div>
      ...
      <div className="flex-1 overflow-auto px-4 pt-5 pb-8 sm:px-7">
        <div>
          {notCandidateReason && (
            <div
              className="mb-5 rounded-xl border border-line px-3 py-3"
              style={{ background: "var(--surface-2)" }}
            >
              <div className="text-xs uppercase tracking-wider text-muted-fg">
                Why not a candidate?
              </div>
              <div className="text-sm text-muted-fg mt-1 leading-relaxed">
                {notCandidateReason}
              </div>
            </div>
          )}
          ...
          <DetailSection title="Video">
            ...
            <DetailRow
              label="Bitrate"
              value={
                file.bitrate_kbps != null
                  ? `${file.bitrate_kbps.toLocaleString()} kbps`
                  : null
              }
              mono
            />
          ...
          <div
            className="mt-4 rounded-xl border border-line px-3 py-3"
            style={{ background: "var(--bg)" }}
          >
            <div className="text-xs uppercase tracking-wider text-muted-dim mb-1">
              Path
            </div>
            <div className="text-xs font-mono text-muted-fg break-all leading-relaxed">
              {file.path}
            </div>
          </div>
        </div>
