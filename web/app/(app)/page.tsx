    ...
    <div className="px-4 pt-5 pb-14 w-full sm:px-7 sm:pt-7">
      ...
      >
        <div
          ...
          style={{
            background:
              "repeating-linear-gradient(0deg, rgba(244,244,244,.022) 0 1px, transparent 1px 4px)",
          }}
        />
        ...
        <div className="mt-6 pt-5 border-t border-line-soft grid grid-cols-3 gap-4 max-sm:grid-cols-1">
          <div>
            ...
            <div className="text-stat font-bold tracking-tight mt-1">
              {formatInt(stats.total_files)}
            </div>
          </div>
          <div>
            <div className="text-xs text-muted-fg uppercase tracking-wider font-bold">
            ...
            <div className="text-stat font-bold tracking-tight mt-1 text-brand">
              {formatInt(candidateCount)}
            </div>
            <div className="text-xs text-muted-dim mt-0.5">
              {formatPct(candidateCount, stats.total_files)} of library
            </div>
          </div>
          <div>
            <div className="text-xs text-muted-fg uppercase tracking-wider font-bold">
            ...
            >
              ...
              <div className="w-[112px] sm:w-[148px] shrink-0 text-right text-muted-fg text-xs tnum">
                ...
                <span className="hidden sm:inline">
                  {" "}
                  ({formatPct(c.file_count, stats.total_files)})
                </span>{" "}
                · {formatBytes(c.total_bytes)}
                <span className="hidden sm:inline">
                  {" "}
                  ({formatPct(c.total_bytes, total)})
                </span>
              </div>
            </div>
          ))}
