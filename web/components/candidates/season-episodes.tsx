...
import { api, type CandidateFilters, type Episode, type MediaFile } from '@/lib/api';
...
export function SeasonEpisodes({
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }: { pageParam: number }) =>
      api.groupedSeasonEpisodes({ ...filters, series: seriesTitle, season, limit: CANDIDATES_PAGE_SIZE, offset: pageParam }),
    initialPageParam: 0,
    getNextPageParam: (lastPage, allPages) => {
      ...
      return lastPage.episodes.length === CANDIDATES_PAGE_SIZE ? loaded : undefined;
    },
  });
